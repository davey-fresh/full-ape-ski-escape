// FULL APE SKI ESCAPE v0.19 — core (tracks, yard-sale, terrain)
...
let obstacles=[],beers=[],particles=[],wipeoutDebris=[],skiTracks=[];
let loseTime=0;
...
let jumpCooldown=0,carveTimer=0,wasAirborne=false;
function newSeed(){terrainSeed=(Math.random()*0x7fffffff)|1}
function resetGame(){newSeed();titusX=400;titusY=140;scrollSpeed=2.9;currentSpeedMod=1;boostTimeLeft=0;jumpVy=0;isAirborne=false;airJumpsLeft=1;mogulSlowLeft=0;shake=0;distance=0;obstacles=[];beers=[];particles=[];wipeoutDebris=[];skiTracks=[];loseTime=0;chaseActive=false;orangutanX=400;orangutanY=-140;orangutanRage=0;lastCloseSfx=0;gameStartTime=Date.now();runStartTime=Date.now();lastSpawn=0;bgOffset=0;beersChugged=0;closeCalls=0;jumpsMade=0;celebTime=0;bros=[];lean=0;leanTarget=0;chugTimer=0;bodyBob=0;jumpCooldown=0;carveTimer=0;wasAirborne=false}
...
function triggerYardSale(x,y){
  ...
  createParticles(x,y,'#fff',36,5.5);createParticles(x,y,'#c8e0f8',18,4);createParticles(x,y,'#ffaa66',10,3);
  ...
  for(const it of items){wipeoutDebris.push({type:it.type,x:x+(Math.random()-0.5)*20,y:y+(Math.random()-0.5)*10,vx:(Math.random()-0.5)*7+it.spread*3,vy:-4-Math.random()*5,rot:(Math.random()-0.5)*Math.PI,rotV:(Math.random()-0.5)*0.35,life:4000+Math.random()*2000});}
}
function doJump(){
...
function leaveTrack(kind){
  const w=kind==='land'?28:(kind==='jump'?16:10+Math.abs(lean)*8);
  skiTracks.push({x:titusX,y:titusY+48,lean:lean,kind,width:w,life:1,age:0});
  if(skiTracks.length>80)skiTracks.shift();
}
